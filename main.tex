...
\vspace{-0.2cm}
\mypara{Summary}
The Internet ecosystem is critically driven by user-perceived {\em QoE} (quality of experience). 
Yet, despite tremendous efforts, many users still suffer from suboptimal QoE. 
We argue that a fundamental reason is that many Internet systems (\eg web backends, CDNs, and ISPs) are unaware of their impact on individual user's QoE; instead, the conventional wisdom is that each system should optimize its own low-level metric,
% (\eg minimizing the delay caused by the individual system), 
 rather than directly optimizing QoE.
In contrast, our overarching thesis is that one can substantially improve QoE and resource efficiency by using real-time per-user QoE information to directly drive the optimization of these subsystems.
