...
\mypara{Summary}
The Internet ecosystem is critically driven by user-perceived {\em quality of experience} ({\em QoE}). 
Yet, many users still suffer from suboptimal QoE, despite tremendous efforts to add more resources to scale-up these Internet services.
We argue that the existing resources are already sufficient to provide much better QoE. 
The key missing piece is that many Internet systems (\eg web backends, CDNs, and ISPs) today are unaware of their and other systems' impact on the QoE perceived by individual users; instead, they focus on optimizing low-level performance metrics.
Our overarching thesis is that one can substantially improve QoE and resource efficiency by using user-perceived QoE to directly drive these systems.
%A fundamental reason is that many Internet systems (\eg web backends, CDNs, and ISPs) are unaware of their impact on individual user's QoE; instead, each system optimizes its own low-level metric,
% (\eg minimizing the delay caused by the individual system), 
% rather than directly optimizing QoE.
%In contrast, our overarching argument is that one can substantially improve QoE and resource efficiency by using real-time per-user QoE information to directly drive the optimization of these subsystems.

This proposal explores the opportunities and challenges of this proposed thesis in the context of web backend systems optimizing web QoE. 
In particular, we introduce a new dimension for optimizing web QoE: the {\em inherent heterogeneity} of how much impact the {\em web backend delay} has on QoE across web requests. 
For instance, a small amount of backend delay can have a much greater impact on the QoE of a web request that has spent only 50ms on wide-area networks than on the QoE of another request that has spent over 800ms on the network.
...
The proposed research will take an integrative approach to design and implement novel QoE-driven web services along three key thrusts:
(1) Quantifying the potential improvement of a QoE-driven web backend system in the wild;
(2) Developing new architectural components to estimate the sensitivity of QoE with respect to the backend processing delay in real time; and
...
These results bear implications beyond web backend---other systems, including content delivery networks, cloud providers, ISPs, cellular carriers, can benefit from the insight as well.

\mypara{Broader impacts}
This project has the potential to improve the experience of web-based applications. 
...
In addition, the proposed project will require significant experimental study, including, measuring web delay and user satisfaction as well as system designs and implementation, which will lead to many opportunities for course enrichment and undergraduate research. 
