...
\mypara{Summary}
User-perceived QoE (quality of experience) is a key driving force behind the ecosystem of web services.
...
This proposal introduces a new dimension for QoE optimization: harnessing the heterogeneity of {\bf sensitivity of QoE to backend delay}.%, \ie how sensitive a user's QoE is to the web backend delay.
...
Such discrepancy in QoE sensitivity is largely ignored in today's web services, but we show that it has profound implications, especially when requests compete for limited resources.
Our early results show that by making existing the web backend aware of QoE sensitivity, we could improve both QoE and resource efficiency than existing solutions.
...
\noindent Large-scale web service providers (such as Microsoft, Facebook, Google) spend tremendous efforts to maintain high user-perceived {\em QoE} (Quality of Experience). 
The fundamental question is how to share resources of the web backend across users so as to optimize their overall QoE. 
...
One of the determining factors of QoE of a web request is its page load time, which generally consists of three types of latency: client-side delay, wide-area network (WANs) delay, and backend delay.
...
{\em backend delay} with the assumption that a backend delay has of $n$~ms has the {\em same} impact on any request.\footnote{Modulo the content-specific (\eg web page type) or user-specific (\eg free vs. premium subscription) factors.}
