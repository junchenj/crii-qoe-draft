\vskip -.75em
\mypara{Summary}
User-perceived quality of experience (QoE) is a driving force behind the Internet ecosystem, yet many users still suffer from suboptimal QoE. 
Unlike previous work (\eg reducing web service delay, pushing caches closer to end users, etc), this proposal introduces a new dimension to QoE optimization: {\bf awareness of QoE sensitivity}, \ie how sensitive a user's QoE is to the delay of the web service.
For instance, a web request that has spent 50ms on wide-area networks before reaching the web server is more likely to be affected by 5ms delay of the web service than another request that has already spent 500ms on the network. 
Such difference of QoE sensitivity is largely not ignored in today's web services, but it has profound implications, especially when the requests compete for limited shared resources.
...
In this proposal, we plan to thoroughly investigate this idea with three steps.
(1) We quantify its potential benefits on QoE and resource savings 
(2) We propose novel algorithms for QoE-aware scheduling and resource allocation in web services. 
(3) We present novel system designs and implementation that makes web services  aware of QoE sensitivity in practice.
